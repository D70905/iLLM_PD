...
PNG_OUT = OUT_DIR / "Fig3_design_response_revised_pillow.png"
PDF_OUT = OUT_DIR / "Fig3_design_response_revised_pillow.pdf"
...
def draw_map_and_lookup(draw, df, box):
    ...
    cx = [tab_box[0], tab_box[0] + 245, tab_box[0] + 520, tab_box[0] + 800, tab_box[0] + 1160, tab_box[2]]
    ...
    for i, r in df.iterrows():
        ...
        vals = [r["study_id"], r["section"], "Flexible" if r["type"] == "flexible" else "Semi-rigid", r["climate"].replace("NoFreeze", "No-freeze"), f"{int(round(r['E_sub']))}"]
...
def draw_structures(draw, df, box):
    ...
    row_base = {"semi_rigid": y0 + 700, "flexible": y0 + 1600}
    ...
    for ptype in ["semi_rigid", "flexible"]:
        ...
        t(draw, (x0 + 24, row_base[ptype] - 300), labels[ptype], F["body"], COL[ptype])
        ...
        for i, r in part.iterrows():
            ...
            for j, (_, c) in enumerate(LAYERS):
                ...
                z += h
            t(draw, (xx + bar_w / 2 + dep / 2, row_base[ptype] + 66), r["study_id"], F["tiny"], anchor="mm")
            t(draw, (xx + bar_w / 2 + dep / 2, row_base[ptype] + 116), f"h={float(r['total_thickness_cm']):.1f} cm", F["micro"], COL["muted"], "mm")
    total_legend_w = 5 * 520
    lx, ly = (x0 + x1 - total_legend_w) / 2, y1 - 58
    for i, (name, c) in enumerate(LAYERS):
        draw.rectangle([lx + i * 520, ly, lx + i * 520 + 50, ly + 30], fill=c)
        t(draw, (lx + i * 520 + 64, ly - 4), name, F["tiny"])
...
def draw_bar_cell(draw, x0, x1, y, val, lo, hi, color, label):
    draw.rectangle([x0, y - 22, x1, y + 22], fill="#eeeeee")
    draw.rectangle([x0, y - 22, x0 + (x1 - x0) * norm(val, lo, hi), y + 22], fill=color)
    t(draw, ((x0 + x1) / 2, y), label, F["table_bold" if label == "1.000" else "table"], anchor="mm")


def draw_matrix(draw, df, box):
    x0, y0, x1, y1 = box
    header(draw, x0, y0 - 78, "c", "LTPP delivered-design metrics")
    metrics = [
        ("E_sub", "Esub\n(MPa)", None),
    ...
    cols = [x0 + 320, x0 + 1120, x0 + 1920, x0 + 2720, x0 + 3520, x0 + 4320]
    bar_w = 650
    yhead = y0 + 70
    t(draw, (x0 + 18, yhead - 24), "Section", F["table_bold"])
    for col, (_, label, _) in zip(cols, metrics):
        for k, line in enumerate(label.split("\n")):
            t(draw, (col + bar_w / 2, yhead - 44 + k * 44), line, F["table_bold"], anchor="mm")
    draw.line([(x0, yhead + 42), (x1, yhead + 42)], fill="#222222", width=3)
    row_h = 78
    for i, r in df.iterrows():
        yy = yhead + 92 + i * row_h
        bg = "#f7fbff" if r["type"] == "flexible" else "#fff8f2"
        draw.rectangle([x0, yy - 34, x1, yy + 34], fill=bg)
        if i == 6:
            draw.line([(x0, yy - 42), (x1, yy - 42)], fill="#777777", width=3)
        c = COL[r["type"]]
        t(draw, (x0 + 18, yy - 21), r["study_id"], F["table"])
        for col, (m, _, mc) in zip(cols, metrics):
            ...
            if m in ["DSR", "SCR"]:
                ...
                lab = f"{val:.1f}"
            draw.rectangle([col, yy - 23, col + bar_w, yy + 23], fill="#eeeeee")
            draw.rectangle([col, yy - 23, col + bar_w * norm(val, lo, hi), yy + 23], fill=c)
            t(draw, (col + bar_w / 2, yy), lab, F["table_bold"], anchor="mm")
    t(draw, (x0, y1 - 10), "Bars are min-max scaled within each metric; costs are USD m-2.", F["micro"], COL["muted"])
...
def draw_ncat(img, draw, real, heat, box):
    ...
    draw.line([(px0, py1), (px1, py1)], fill="#222222", width=3)
    draw.line([(px0, py0), (px0, py1)], fill="#222222", width=3)
    draw.line([(px1, py0), (px1, py1)], fill="#222222", width=3)
    for tick in [0, 0.2, 0.4, 0.6]:
        yy = py1 - (py1 - py0) * tick / 0.6
        draw.line([(px0, yy), (px1, yy)], fill="#e8e8e8", width=1)
    ...
    for i, r in real.iterrows():
        ...
        t(draw, (cx, py1 - bar_h - 50), f"{reduction:.0f}%", F["axis"], "#252B48", "mm")
    ...
    draw.line(as_pts, fill=COL["semi_rigid"], width=5)
    draw.line(alt_pts, fill="#0072B2", width=6)
    for x, y in as_pts:
        draw.ellipse([x - 12, y - 12, x + 12, y + 12], fill=COL["semi_rigid"], outline="white", width=2)
    for x, y in alt_pts:
        draw.ellipse([x - 14, y - 14, x + 14, y + 14], fill="#0072B2", outline="white", width=2)

    lx, ly = px0 + 80, py0 + 46
    draw.rectangle([lx, ly - 20, lx + 48, ly + 18], fill="#4C5C92")
    t(draw, (lx + 64, ly - 24), "Rutting-related response reduction", F["legend"])
    draw.line([(lx + 840, ly), (lx + 930, ly)], fill=COL["semi_rigid"], width=5)
    draw.ellipse([lx + 885 - 12, ly - 12, lx + 885 + 12, ly + 12], fill=COL["semi_rigid"])
    t(draw, (lx + 950, ly - 24), "As-built DSR", F["legend"])
    draw.line([(lx + 1225, ly), (lx + 1315, ly)], fill="#0072B2", width=6)
    draw.ellipse([lx + 1270 - 14, ly - 14, lx + 1270 + 14, ly + 14], fill="#0072B2")
    t(draw, (lx + 1338, ly - 24), "Candidate DSR", F["legend"])
...
def main():
    ...
    draw_structures(draw, df, (230, 2040, 5170, 3920))
    draw_matrix(draw, df, (230, 4140, 5170, 5290))
    draw_ncat(img, draw, real, heat, (230, 5580, 5170, 7380))
