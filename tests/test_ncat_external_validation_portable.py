...
class NcatExternalValidationTests(unittest.TestCase):
    ...
    def test_case_json_integrity(self):
        payload = json.loads((PROJECT / "experiments/ncat_data/ncat_cases.json")
                             .read_text(encoding="utf-8"))
        self.assertEqual(len(payload["cases"]), 7)
        self.assertEqual(len({c["section_id"] for c in payload["cases"]}), 7)
        for case in payload["cases"]:
            validation.validate_case(case)

    def test_existing_summary_integrity(self):
        with (PROJECT / "ncat_mepdg_rut_summary.csv").open(encoding="utf-8-sig") as f:
            rows = list(csv.DictReader(f))
        self.assertEqual(len(rows), 7)
        for row in rows:
            self.assertTrue(np.isfinite(float(row["pred_total_mm"])))
            self.assertTrue(np.isfinite(float(row["meas_rut_mm"])))
